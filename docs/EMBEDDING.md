...
**رفتار دکمه:** هر `[data-tryon-open]` یک overlay تمام‌صفحهٔ *مشترک* باز می‌کند و محصول متناظر با `id` یا `sku` داخل کاتالوگ فروشگاه را انتخاب می‌کند؛ اگر SKU متعلق به یک رنگ/واریانت باشد، همان رنگ هم فعال می‌شود. کاتالوگ فروشنده با کاتالوگ نمونه جایگزین نمی‌شود. دکمه‌های ساخته‌شده با AJAX نیز به‌صورت خودکار کار می‌کنند. اگر می‌خواهید دکمه به یک اسلات inline مشخص برود:
...
| `data-tryon-sku` | با `id` یا SKU محصول شروع کن؛ اگر واریانت‌های رنگ `sku` دارند، همان رنگ نیز انتخاب می‌شود. شناسه‌های عددی هم به‌صورت رشته مقایسه می‌شوند |
